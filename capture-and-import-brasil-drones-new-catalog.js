...
if (!approved) throw new Error('Importação real bloqueada: defina BRASIL_DRONES_IMPORT_APPROVED=true.');
...
const server = http.createServer((request, response) => {
  ...
  response.end('<h1>Autorizado</h1><p>A importação foi iniciada. Pode voltar ao Codex.</p>');

  const child = spawn(process.execPath, ['scripts/bling/import-brasil-drones-new-catalog.mjs'], {
    cwd: ROOT,
    env: { ...process.env, BLING_AUTH_CODE: code, DRY_RUN: 'false', BRASIL_DRONES_IMPORT_APPROVED: 'true' },
    stdio: 'inherit',
  });
