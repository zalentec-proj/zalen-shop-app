...
const server = http.createServer((request, response) => {
  ...
  console.log(`Autorização recebida. Executando em modo ${dryRun ? 'dry-run' : 'real'}...`);
  runCategoryJob(code);
});

server.listen(PORT, '127.0.0.1', () => {
  console.log(`Callback local: http://localhost:${PORT}${CALLBACK_PATH}`);
  console.log('Use este callback no app Bling exclusivo da Brasil Drones.');
  console.log('Abra esta URL de autorização:');
  console.log(authorizeUrl.toString());
});

function runCategoryJob(code) {
  const args = ['scripts/bling/create-brasil-drones-model-categories.mjs'];
  ...
  const child = spawn(process.execPath, args, {
    ...
    env: {
      ...process.env,
      BLING_CUSTOMER_CLIENT_ID: clientId,
      BLING_CUSTOMER_CLIENT_SECRET: clientSecret,
      BLING_AUTH_CODE: code,
      DRY_RUN: String(dryRun),
      MODEL_CATEGORIES_APPROVED: String(approved),
    },
    stdio: 'inherit',
  });
  child.on('error', (error) => {
    console.error(`Falha ao iniciar criação de categorias: ${error.message}`);
