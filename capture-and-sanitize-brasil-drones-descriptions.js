...
const server = http.createServer((request, response) => {
  ...
  response.end(
    auditOnly
      ? '<h1>Autorizado</h1><p>A auditoria das descrições foi iniciada. Pode voltar ao Codex.</p>'
      : '<h1>Autorizado</h1><p>A correção das descrições foi iniciada. Pode voltar ao Codex.</p>'
  );

  const child = spawn(
    process.execPath,
    ['scripts/bling/sanitize-brasil-drones-descriptions.mjs'],
    {
      cwd: ROOT,
      env: { ...process.env, BLING_AUTH_CODE: code },
      stdio: 'inherit',
    }
  );
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`Modo: ${auditOnly ? 'AUDITORIA SEM ALTERAÇÕES' : 'CORREÇÃO DE DESCRIÇÕES'}`);
