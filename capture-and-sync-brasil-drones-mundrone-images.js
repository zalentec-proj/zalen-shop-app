...
const server = http.createServer((request, response) => {
  ...
  response.end('<h1>Autorizado</h1><p>A atualização das imagens foi iniciada no app privado da Brasil Drones.</p>');

  const child = spawn(process.execPath, ['scripts/bling/sync-brasil-drones-mundrone-images.mjs'], {
    cwd: ROOT,
    env: {
      ...process.env,
      BLING_AUTH_CODE: code,
      MUNDRONE_IMAGE_MODE: 'update',
      BRASIL_DRONES_IMAGE_UPDATE_APPROVED: 'true',
    },
    stdio: 'inherit',
  });
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log('Use apenas o app Bling privado da Brasil Drones.');
