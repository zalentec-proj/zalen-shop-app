...
const server = http.createServer((request, response) => {
  ...
  response.end(
    '<h1>Autorizado</h1><p>A correção de unidade para centímetros foi iniciada. Pode voltar ao Codex.</p>'
  );
  const child = spawn(
    process.execPath,
    ['scripts/bling/apply-brasil-drones-dimension-unit-corrections.mjs'],
    {
      cwd: ROOT,
      env: {
        ...process.env,
        BLING_AUTH_CODE: code,
        BRASIL_DRONES_DIMENSION_UPDATE_APPROVED: 'true',
      },
      stdio: 'inherit',
    }
  );
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log('Escopo: somente unidade de medida de metros para centímetros.');
