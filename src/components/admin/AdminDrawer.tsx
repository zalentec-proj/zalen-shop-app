...
export function AdminDrawer({
  ...
  return (
    <div className={`fixed inset-0 z-50 flex bg-[#020713]/75 backdrop-blur-sm ${presentation === 'modal' ? 'items-center justify-center p-4' : 'justify-end'}`} onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
      <section ref={panelRef} role="dialog" aria-modal="true" aria-labelledby="admin-drawer-title" className={presentation === 'modal' ? 'max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl border border-white/10 bg-[#081225] shadow-[0_24px_80px_rgba(0,0,0,.5)]' : 'h-full w-full max-w-xl overflow-y-auto border-l border-white/10 bg-[#081225] shadow-[-24px_0_80px_rgba(0,0,0,.45)]'}>
