...
function ModelPicker({ product, models }: { product: ProductRow; models: ModelOption[] }) {
  ...
  return <AdminActionForm action={saveProductDroneModelsAction} successMessage="Compatibilidade salva com sucesso." pendingMessage="Salvando compatibilidade…" className="space-y-4"><input type="hidden" name="productId" value={product.id}/>{suggestion.length?<button type="button" onClick={()=>setSelected(new Set(product.suggestedModelIds))} className="flex w-full items-start gap-2 rounded-lg border border-amber-400/20 bg-amber-400/5 p-3 text-left text-xs text-amber-200"><Sparkles className="mt-0.5 h-4 w-4 shrink-0"/><span><strong className="block">Aplicar sugestão</strong>Usar os modelos detectados no nome do produto.</span></button>:null}<div className="max-h-[55vh] space-y-3 overflow-y-auto pr-1">{Array.from(groups).map(([line,options])=><fieldset key={line} className="rounded-lg border border-white/7 p-3"><legend className="px-1 text-[10px] font-semibold uppercase tracking-wider text-slate-500">{line}</legend><div className="grid gap-1 sm:grid-cols-2">{options.map((model)=><label key={model.id} className="flex cursor-pointer items-center gap-2 rounded-md px-2 py-2 text-xs text-slate-300 hover:bg-white/5"><input type="checkbox" name="modelIds" value={model.id} checked={selected.has(model.id)} onChange={()=>setSelected((current)=>{const next=new Set(current);next.has(model.id)?next.delete(model.id):next.add(model.id);return next;})}/>{model.label}</label>)}</div></fieldset>)}</div><button className="w-full rounded-lg bg-blue-600 px-4 py-2 text-xs font-semibold disabled:cursor-wait disabled:opacity-60">Salvar compatibilidade</button></AdminActionForm>;
}
