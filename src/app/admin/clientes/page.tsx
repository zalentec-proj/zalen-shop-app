...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<CustomerParams> }) {
  ...
  return <AdminPageFrame>
    ...
    {params.new === 'customer' ? <AdminDrawer parameter="new" presentation="modal" title="Novo cliente" description="Cadastre apenas os dados essenciais."><form action={createAdminCustomerAction} className="grid gap-3"><label className="grid gap-1 text-xs">Nome<input required name="name" className="h-9 rounded-lg border border-white/8 bg-[#050A14] px-3" /></label><label className="grid gap-1 text-xs">E-mail<input type="email" name="email" className="h-9 rounded-lg border border-white/8 bg-[#050A14] px-3" /></label><label className="grid gap-1 text-xs">Telefone<input name="phone" className="h-9 rounded-lg border border-white/8 bg-[#050A14] px-3" /></label><label className="grid gap-1 text-xs">CPF ou CNPJ<input name="document" className="h-9 rounded-lg border border-white/8 bg-[#050A14] px-3" /></label><label className="grid gap-1 text-xs">Observações<textarea name="notes" rows={3} className="rounded-lg border border-white/8 bg-[#050A14] p-3" /></label><button className="mt-2 rounded-lg bg-blue-600 px-4 py-2 text-xs font-semibold">Cadastrar cliente</button></form></AdminDrawer> : null}
