...
const server = http.createServer((request, response) => {
  ...
  if (error || !code || returnedState !== state || handled) {
    ...
    response.end(error ?? 'OAuth callback invalido.');
    return;
  }
  ...
  console.log('Code recebido. Atualizando nomes originais...');
  runUpdate(code);
});
...
function runUpdate(code) {
  const child = spawn(process.execPath, ['update-product-names.js'], {
    cwd: ROOT,
    env: {
      ...process.env,
